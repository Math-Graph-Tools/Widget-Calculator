...
const CyberLoader = () => html`
  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-cyber-black overflow-hidden animate-fade-in">
    ...
    <div className="absolute top-10 left-10 w-16 h-16 border-t-2 border-l-2 border-cyber-neon/50"></div>
    <div className="absolute top-10 right-10 w-16 h-16 border-t-2 border-r-2 border-cyber-neon/50"></div>
    <div className="absolute bottom-10 left-10 w-16 h-16 border-b-2 border-l-2 border-cyber-neon/50"></div>
    <div className="absolute bottom-10 right-10 w-16 h-16 border-b-2 border-r-2 border-cyber-neon/50"></div>
...
export const GamePlayer = ({ game, onBack }) => {
  ...
  useEffect(() => {
    ...
    // For native games, simulate a loading delay to show the nice animation
    if (game.type === GameType.NATIVE) {
        const timer = setTimeout(() => {
            setIsLoading(false);
        }, 2500); // 2.5s simulated load time for dramatic effect
        return () => clearTimeout(timer);
    }
  }, [game.id, game.type]); // Depend on ID to reset when switching games
    ...
    <div className="animate-fade-in flex flex-col h-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 bg-cyber-slate/20 p-4 border border-cyber-slate backdrop-blur-sm gap-4">
        <div className="flex items-center space-x-4 w-full sm:w-auto justify-between sm:justify-start">
          <${Button} onClick=${onBack} variant="secondary" size="sm" icon=${html`<span>←</span>`}>
            ABORT
          </${Button}>
        ...
        <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
           <${Button} onClick=${toggleFullscreen} variant="secondary" size="sm" className="hidden md:inline-flex">
           ...
           ${!isNative && html`
             <${Button} 
               ...
               title="Open in new tab if game fails to load"
               className="whitespace-nowrap shadow-neon hover:scale-105 transition-transform"
             >
               OPEN IN NEW TAB ↗
             </${Button}>
           `}
      ...
      <div 
        id="game-container" 
        className=${`relative w-full overflow-hidden bg-black border border-cyber-slate shadow-neon-pink/10 flex-grow flex flex-col ${isFullscreen ? 'p-0 border-none' : 'aspect-video md:aspect-auto border-t-2 border-b-2 border-cyber-neon/50'}`}
      >
        ...
        ` : html`
          <iframe
            ...
            src=${game.url}
            title=${game.title}
            className=${`w-full h-full border-0 flex-grow transition-opacity duration-1000 ${isLoading ? 'opacity-0 absolute' : 'opacity-100'}`}
            ...
            allowFullScreen
            onLoad=${() => {
               // Add a small delay to ensure visual smoothness for iframes
               // If it's native, the useEffect handles it. If iframe, this handles it.
               if (!isNative) {
      ...
      <div className="mt-6 text-slate-400 bg-cyber-slate/10 p-6 border-l-4 border-cyber-neon">
        ...
        <p className="mt-2 text-xs text-cyber-neon font-mono font-bold">
          ${isNative 
            ? '// SYSTEM ALERT: NATIVE SIMULATION LOADED. OPTIMAL PERFORMANCE GUARANTEED.'
            : '// SYSTEM ALERT: IF SCREEN REMAINS BLACK OR BLOCKED, CLICK "OPEN IN NEW TAB" ABOVE TO BYPASS PROTOCOLS.'
          }
        </p>
      </div>
    </div>
